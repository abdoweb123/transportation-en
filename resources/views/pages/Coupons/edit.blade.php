    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <!-- add_form -->
                <form action="{{ route('coupons.update', $item->id) }}" method="post">
                    {{ method_field('patch') }}
                    @csrf
                    <div class="row">
                        <div class="col">
                            <label for="name_ar" class="mr-sm-2">الكود:</label>
                            <input id="name" type="text" name="code" value="{{$item->code}}" class="form-control">
                        </div>
                        <div class="col">
                            <label class="mr-sm-2" style="display:block">الخصم:</label>
                            <input type="number" step="0.1" class="form-control" name="amount" value="{{$item->amount}}" style="display:inline-block; width:75%">
                    ...
                    <div class="row max_amount" style="@if($item->percent == 0) display:block; @else display:none; @endif">
                        <div class="col">
                            <label for="name_ar" class="mr-sm-2">أكبر قيمة للخصم:</label>
                            <input id="name" type="number" step="0.1" name="max_amount"  value="{{$item->max_amount}}" class="form-control">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col">
                            <label for="name_ar" class="mr-sm-2">تاريخ البداية:</label>
                            <input id="name" type="date" name="startDate"  value="{{$item->startDate}}" class="form-control">
                        </div>
                        <div class="col">
                            <label for="name_ar" class="mr-sm-2">تاريخ النهاية:</label>
                            <input id="name" type="date" name="endDate"  value="{{$item->endDate}}" class="form-control">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col">
                            <label for="name_ar" class="mr-sm-2">العدد الكلي للأشخاص:</label>
                            <input id="name" type="text" name="max_users"  value="{{$item->max_users}}" class="form-control">
                        </div>
                        <div class="col">
                            ...
                            <select name="trips[]" multiple class="form-control trips" style="padding:10px; width:75%; display:inline-block">
                                @foreach($trips as $trip)
                                    <option value="{{$trip->id}}"  @foreach ($item->couponTrips as $tripData) {{$trip->id == $tripData->tripData_id ? 'selected' : ''}}  @endforeach>{{$trip->name}}</option>
                                @endforeach
                            </select>
