...
class SupplierContractRouteImport implements ToCollection ,WithHeadingRow //, WithChunkReading, ShouldQueue
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            $contract=ContractSublier::where('name',$row['contract'])->first();
        ...
        if ($bus_type != null && $discount != null && $contract != null) {
                $check=SublierCotractRoute::where(['suplier_contract_id'=>$contract->id,'suplier_id'=>$supplier_id,'route_id'=>$route_id,'bus_type_id'=>$bus_type->id,'service_type_id'=>$service_type_id])->first();
                if ($check == null) {
                    $data=new SublierCotractRoute;
                }else{
                    $data=$check;
                }
                $data->suplier_contract_id=$contract->id;
